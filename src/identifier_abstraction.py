...
import nltk # required version 3.6.2
...
long_ops_pattern = "|".join([re.escape(op).replace("/", "\/") for op in long_ops])
...
STRING_LIT_MARKER = "__x_STRING_LIT__"
CHAR_LIT_MARKER = "__x_CHAR_LIT__"

BASE_LONG_OP_MARKER = "x_LONG_OP"
LONG_OP_MARKER_TEMPLATE = f" __{BASE_LONG_OP_MARKER}_%s__ "


def symbolic_tokenize(code):
    raw_line_len = len(code.splitlines())
    while True:
        match = re.search(long_ops_pattern, code)
        if match is None:
            break
        match_span = match.span()
        op = re.sub("\s+", "", match.group())
        op_index = long_ops.index(op)
        code = code[:match_span[0]] + LONG_OP_MARKER_TEMPLATE % op_index + code[match_span[1]:]

    code = re.sub(r"'(\n|[^'\n]{,5})'", f" {CHAR_LIT_MARKER} ", code)
    code = re.sub(r"\"(\\.|[^\"])*\"", f" {STRING_LIT_MARKER} ", code)
    preprocessed_line_len = len(code.splitlines())
    assert raw_line_len == preprocessed_line_len, str(raw_line_len) + " " + str(preprocessed_line_len) + "\n" +code
    ...
    for line in code.splitlines():
        ...
        for idx in range(len(c_tokens)):
            ...
            if t in keywords:
                ...
                final_tokens.append(t)
            elif BASE_LONG_OP_MARKER in t:
                final_tokens.append(t)
            elif t == CHAR_LIT_MARKER:
                final_tokens.append("CHAR_LIT")
            elif t == STRING_LIT_MARKER:
                final_tokens.append("STRING_LIT")
            elif re.match("^[0-9]+(\.[0-9]+)?$", t) is not None:
                final_tokens.append("NUMBER_LIT")
            elif re.match("^[0-9]*(\.[0-9]+)$", t) is not None:
                final_tokens.append("NUMBER_LIT")
